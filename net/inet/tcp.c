...
void tcp_build_options(struct sk_buff *skb, int if_mss, int mss, int if_ts, int if_sack, \
                          int if_wscale, int wacale, unsigned int tstamp, unsigned int ts_recent)
{
    struct options_packed_ptr *ptr;
    struct tcphdr *tcph;
    int doff = SIZEOF_TCPHDR;

    tcph = get_tcph(skb);
    ptr = (struct options_packed_ptr *)(tcph + 1);
    
    if (if_mss)
    {
        ptr->value = htonl((TCPOPT_MSS << 24) | (TCPOLEN_MSS << 16) | mss);
		ptr++;
    ...
    if (if_ts)
    {
        if (if_sack)
        {
            ptr->value = htonl((TCPOPT_SACK_PERM  << 24) |\
        ...
        {
			ptr->value = htonl((TCPOPT_NOP << 24) |
				       (TCPOPT_NOP << 16) |
				       (TCPOPT_TIMESTAMP << 8) |
				       TCPOLEN_TIMESTAMP);
			ptr++;
        }
		ptr->value = htonl(tstamp);		
		ptr++;
		
		ptr->value = htonl(ts_recent);	
		ptr++;
